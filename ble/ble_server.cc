...
void BleServer::initialize(const char* deviceName, BleServerCallbacks* pBleServerCallbacks) {
	...
	ble_uart_server_SetCallbackConnection(bleCallbackConnection, bleCallbackMTU);
	...
	// NOTE: process_message_task in ble_uart_server.c is used instead.
...
{
	// Connected
	if (ble_uart_server_ClientConnected())
	{
...
static void receive_Task(void *pvParameters)
{
	...
	while(1)
	{
		...
		logI("Queue received our message!!");
	...
	for (uint8_t i = 0; i < size; i++)
	{
		...
		if (character == '\n') // New line
		{
			if (mLineBuffer.length() > 0) // Not empty ?
			{
				...
				// Send message to queue, wait if it is full
